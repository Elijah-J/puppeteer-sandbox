/**
 *
 * Originally sourced from: https://github.com/puppeteer/examples/blob/master/crawlsite.js
 *
 * Below is the original script attribution.
 *
...
const VIEWPORT = SCREENSHOTS
  ? { width: 1028, height: 800, deviceScaleFactor: 2 }
  : null;
...
async function crawl(browser, page, depth = 0) {
  ...
  if (crawledPages.has(page.url)) {
    ...
    if (SCREENSHOTS) {
      ...
      await fsPromises.writeFile(path, imgBuff); // async
...
(async () => {
  ...
  await deleteAsync([`${OUT_DIR}/*`]); // cleanup after last run.
